...
onload = function ()
{
    ...
    const contact_list = new Trie(); //making a Trie Data Structure 

    add.onclick = function () {
        let details = contact_info.value;
        details = details.split(',');
        if(details.length!==2){
            alert("Please Enter Corrct format");
            return;
        }
        details[0] = details[0].trim(); // to trim the spaces 
        details[1] = details[1].trim(); // to trim the spaces 
    ...
    del.onclick = function () {
        ...
        if(details.length!==10){
            alert("Contact Number You Entered is not contains 10 digits");
            return;
        }
